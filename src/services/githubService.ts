...
import type { Repo, FileNode, GetReposOptions, CreateRepoOptions } from '../types.ts';
import { logEvent, logError, measurePerformance } from './telemetryService.ts';

export const getRepos = async (options: GetReposOptions = {}): Promise<Repo[]> => {
    return measurePerformance('getRepos', async () => {
        logEvent('getRepos_start', options);
        try {
            ...
            logError(error as Error, { context: 'getRepos', options });
            throw new Error(`Failed to fetch repositories: ${(error as Error).message}`);
        }
    });
};

export const getPublicReposForUser = async (username: string, options: GetReposOptions = {}): Promise<Repo[]> => {
    return measurePerformance('getPublicReposForUser', async () => {
        logEvent('getPublicReposForUser_start', { username, ...options });
        try {
            const octokit = getOctokit();
            ...
            logEvent('getPublicReposForUser_success', { count: data.length });
            return data as Repo[];
        } catch (error) {
            logError(error as Error, { context: 'getPublicReposForUser', username, options });
            throw new Error(`Failed to fetch repositories for ${username}: ${(error as Error).message}`);
        }
    });
};

export const createRepo = async (options: CreateRepoOptions): Promise<Repo> => {
    return measurePerformance('createRepo', async () => {
        logEvent('createRepo_start', { name: options.name });
        try {
            const octokit = getOctokit();
            const { data } = await octokit.request('POST /user/repos', {
                ...options
            });
            logEvent('createRepo_success', { repo: data.full_name });
            return data as Repo;
        } catch (error) {
            logError(error as Error, { context: 'createRepo', options });
